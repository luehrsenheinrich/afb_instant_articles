...
if ! $(${WP_CLI} core is-installed); then
	...
	docker-compose run --u=root --rm cli chown www-data -R /var/www/
	...
	${WP_CLI} core install --url=localhost --title="AFBIA Test" --admin_user=wordpress --admin_password=wordpress --admin_email=test@test.com

	# Check for WordPress updates, just in case the WordPress image isn't up to date.
	${WP_CLI} core update

	# Import and activate needed plugins
	${WP_CLI} plugin install gutenberg wordpress-importer query-monitor debug-bar theme-check --activate
  ${WP_CLI} plugin activate allfacebook-instant-articles
